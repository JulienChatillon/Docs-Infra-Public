...
* **Trafic vers le partenaire (`<ZONE_FRANCOIS>`) :** Pas de traduction (No-NAT). Les IP sources de la zone Serveurs (`<RESEAU_SERVEURS_LABO>.x`) sont conservées pour permettre le routage de retour via le VPN de production.
...
| ✅ Autoriser | IPv4 | `<ZONE_FRANCOIS>` | `<ZONE_SERVEURS>` | Laisse entrer le trafic du VPN Site-to-Site (<PEER_S2S>) spécifiquement destiné à la zone **SERVEURS**. |
